...
def main(d2m_file, sp_file, tmpdir, outdir):
    ...
    sphum_str = (tmpdir / d2m_file.name.replace('d2m', 'sphum')).as_posix() # assuming d2m in name
    run_cmd(f'gcp {d2m_str} {tmp_str}')
    run_cmd(f'gcp {sp_str} {tmp_str}')
    run_cmd(f'cdo expr,"svp=611.2*exp(17.67*(d2m-273.15)/(d2m-29.65))" {(tmpdir / d2m_file.name).as_posix()} {svp_str}')
    run_cmd(f'cdo -expr,"_mr=0.622*svp/(sp-svp);sphum=_mr/(1+_mr);" -merge {svp_str} {(tmpdir / sp_file.name).as_posix()} {sphum_str}')
