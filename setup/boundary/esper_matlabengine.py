...
for segment in [1, 2, 3]:
    ...
    for yr in [2022, 2023, 2024]:
        ...
        ny = len(lat)
        nx = len(lon)
        out_coords = np.vstack([np.vstack([lon.values, lat.values, np.repeat(z, nx)]).T for z in glorys_z.values])
        pred_vars = np.vstack([np.vstack([ave_salt.squeeze()[i, :], ave_temp.squeeze()[i, :]]).T for i in range(nz)])
        ...
        esper_result = eng.eval("ESPER_LIR(desired_vars, output_coords, pred_vars, predictor_types, 'Equations', equations, 'EstDates', est_dates)")

        alk = np.array(esper_result['TA']).reshape([-1, nz], order='F') * 1e-6
        dic = np.array(esper_result['DIC']).reshape([-1, nz], order='F') * 1e-6

        h_dims = [f'nx'+segstr, f'ny'+segstr]
        long_dim = h_dims[0] if ave_temp.sizes[h_dims[0]] > ave_temp.sizes[h_dims[1]] else h_dims[1]
        short_dim = next((d for d in h_dims if d != long_dim))
        z_dim = f'nz'+segstr

        ds = xarray.Dataset(
            {'alk'+segstr: ((long_dim, z_dim), alk), 'dic'+segstr: ((long_dim, z_dim), dic)}, 
        ...
        ds['dz_alk'+segstr] = temp['dz_thetao'+segstr].isel(time=0).drop_vars('time')
    ...
    encoding = {'time': {'calendar': 'gregorian', 'dtype': 'float64', '_FillValue' :1.0e20}}
    all_years.to_netcdf(
