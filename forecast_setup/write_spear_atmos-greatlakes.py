...
def write_atmos(
    ...
    if rerun or not out_dir.is_dir():
        ...
        static = xarray.open_dataset('/work/pja/spear/atmos.static.nc')
...
#            ds[main_var] = ds[main_var].where(is_ocean)      # Commented out for the Great Lakes, which is part of land model - PA 9/5/2025
            padded = pad_ds(ds)
            fout = tmpdir / f.name
            write_ds(padded, fout)
            # cdo doesn't like if the input is also the output here 
#            subprocess.run(                                  # Commented out for the Great Lakes - PA 9/5/2025
#                [
...
#            fout.with_suffix(fout.suffix + '.new').rename(fout)  # Commented out for Great Lakes
...
def write_atmos_all_members(
    ...
    # Read mask for flooding
    static = xarray.open_dataset('/work/pja/spear/atmos.static.nc') # PA: I copied /work/acr/spear/atmos.static.nc to my /work/ directory
    is_ocean = np.invert(static.land_mask.astype('bool'))
    members = {}
    ...
    for ens, source_files in members.items():
        ...
        for f in extracted_files:
            ...
            # coordinate/metadata variables
            main_var = next(x for x in ds.data_vars if len(ds[x].dims) == 3)
#            ds[main_var] = ds[main_var].where(is_ocean)            # Commented out for Great Lakes - PA
            padded = pad_ds(ds)
            fout = tmpdir / f.name
            write_ds(padded, fout)
            # cdo doesn't like if the input is also the output here
#            subprocess.run(                                       # Commented out for Great Lakes - PA
#                [
...
#            )
#            # Rename the file generated by cdo to the output file
#            fout.with_suffix(fout.suffix + '.new').rename(fout) # Commented out for Great Lakes - PA
            logger.info('vftmp -> work')
            out_dir.mkdir(exist_ok=True)
            subprocess.run(
